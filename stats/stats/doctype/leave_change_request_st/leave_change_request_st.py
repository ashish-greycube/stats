...
class LeaveChangeRequestST(Document):
	...
	def validate_leave_dates(self):
		...
		if (self.extend_date and getdate(self.extend_date) <= getdate(la.from_date)) or (self.stop_date and getdate(self.stop_date) <= getdate(la.from_date)):
		...
		if self.change_type == 'Stop Vacation':
			...
			if less_days and less_days > 0:
				...
				if less_days_date >= getdate(self.stop_date):
					frappe.throw(_("You cann't stop leave before {0}").format(less_days_date))
