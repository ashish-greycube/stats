...
from frappe.utils import date_diff, add_to_date, get_datetime, get_date_str, cstr, time_diff_in_hours
...
class BusinessTripRequestST(Document):
	...
	def create_future_attendance_for_business_trip_time(self):
		if self.status == "Approved":
			...
			for fiscal_year in range ((self.business_trip_start_date).year, (self.business_trip_end_date).year+1):
